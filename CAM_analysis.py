# -*- coding: utf-8 -*-
"""LithoBench_Ad-CAM.ipynb의 사본의 사본

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1gR25HyCXa8aoUj8hdc9H0Qq4hbs_74KS
"""

# -*- coding: utf-8 -*-
"""
One‑shot script for CAM visualisation on DAMO‑ILT.
Only Grad‑CAM / Score‑CAM / Adaptive‑CAM are used.
Ablation‑CAM code paths have been removed to save GPU time.
"""
...
from lithobench.dataset import loadersLitho, loadersILT, loadersAllLitho   # noqa: F401  (kept for compatibility)
import lithobench.evaluate as evaluate                                     # noqa: F401
...
# --------------------------------------------------
# 1. Data‑set helpers (unchanged variable names)
# --------------------------------------------------
...
def resize(tensor, out_size=global_size):
    """Tensor → bilinear resize → numpy (no threshold)."""
...
def get_cam_maps_conv_tail(model, input_tensor, label_tensor, target_layer_):
    ...
    Returns the same tuple structure as before, with `ablation_map` replaced by
    ...
    ad_cam = ad_cam[0, 0].detach().cpu().numpy()          # ← 핵심 한 줄
    ad_cam = (ad_cam - ad_cam.min()) / (ad_cam.max() + 1e-8)

    # `ablation_map` placeholder retained for backward‑compatibility
    ablation_map = None

    return (gradcam_map, ablation_map, scorecam_map,
...
# --------------------------------------------------
# 3. Single‑sample test & visualisation
# --------------------------------------------------
def testSingleMaskOpt(folder, model):
    ...
    for target_layer in target_layers:
        ...
        for glp, target_png, pixel_png in zip(filesGLP, filesTarget, filesPixel):
            ...
            # --- Post‑processing & masks (same as original)
...
# --------------------------------------------------
# 4. Main entry (same CLI paths as before)
# --------------------------------------------------
if __name__ == "__main__":
    ...
    netG_path = "/content/drive/MyDrive/Colab Notebooks/LothoBench/DAMO_ILT_netG.pth"
    netD_path = "/content/drive/MyDrive/Colab Notebooks/LothoBench/DAMO_ILT_netD.pth"
    ...
    # 3) 테스트 폴더 경로
    folder_test = "/content/drive/MyDrive/Colab Notebooks/LothoBench/TestImage/Mask_Optimization_Test"
